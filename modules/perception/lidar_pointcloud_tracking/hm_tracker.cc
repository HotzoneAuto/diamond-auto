...
#include "modules/perception/lidar_pointcloud_tracking/feature_descriptor.h"
#include "modules/perception/lidar_pointcloud_tracking/hungarian_matcher.h"
#include "modules/perception/lidar_pointcloud_tracking/kalman_filter.h"
#include "modules/perception/lidar_pointcloud_tracking/track_object_distance.h"
...
HmObjectTracker::HmObjectTracker() {
  ...
  config_.name = "HmObjectTracker";
  config_.version = "1.1.0";
  config_.matcher_method = "HUNGARIAN_MATCHER";
  config_.filter_method = "KALMAN_FILTER";
  config_.track_cached_history_size_maximum = 5;
  config_.track_consecutive_invisible_maximum = 2;
  config_.track_visible_ratio_minimum = 0.6;
  config_.collect_age_minimum = 0;
  config_.collect_consecutive_invisible_maximum = 0;
  config_.acceleration_noise_maximum = 5;
  config_.speed_noise_maximum = 0.4;
  config_.match_distance_maximum = 4.0;
  config_.location_distance_weight = 0.6;
  config_.direction_distance_weight = 0.2;
  config_.bbox_size_distance_weight = 0.1;
  config_.point_num_distance_weight = 0.1;
  config_.histogram_distance_weight = 0.5;
  config_.histogram_bin_size = 10;
  config_.use_adaptive = true;
  config_.measurement_noise = 0.4;
  config_.initial_velocity_noise = 5;
  config_.xy_propagation_noise = 10;
  config_.z_propagation_noise = 10;
  config_.breakdown_threshold_maximum = 10.0;
