...
int CWrLs1207DEParser::Parse(char *data, size_t data_length, WrLsConfig &config,
  ...
  ADEBUG << "Publishing with frame id: " << config.frame_id.c_str();
  ...
  msg.header.timestamp_sec =
      start_time - ros::Duration().fromSec(dataCount * msg.time_increment);
  /* - shit forward to time of first published scan point*/
  msg.header.timestamp_sec +=
      ros::Duration().fromSec((double)index_min * msg.time_increment);
  /* - add time offset (to account for USB latency etc.)*/
  msg.header.timestamp_sec += ros::Duration().fromSec(config.time_offset);
  ...
  if (fabs(expected_time_increment - msg.time_increment) > 0.00001) {
    ROS_DEBUG_THROTTLE(
        60,
        "The time_increment, scan_time and angle_increment values reported by "
        "the scanner are inconsistent! "
        "Expected time_increment: %.9f, reported time_increment: %.9f. "
        "Perhaps you should set the parameter time_increment to the expected "
