...
class Id0x0c0ba7f0 : public ::apollo::drivers::canbus::ProtocolData<
 ...
 private:
  ...
  // 'fMotMaxPower', 'offset': 0.0, 'order': 'intel', 'physical_range':
  // '[0|255]', 'physical_unit': 'kw', 'precision': 1.0, 'type': 'int'}
  int fmotmaxpower(const std::uint8_t* bytes, const int32_t length) const;

  // config detail: {'bit': 8, 'is_signed_var': False, 'len': 8, 'name':
  // 'fMotMaxSpd', 'offset': 0.0, 'order': 'intel', 'physical_range':
  // '[0|25500]', 'physical_unit': 'rpm', 'precision': 100.0, 'type': 'double'}
  double fmotmaxspd(const std::uint8_t* bytes, const int32_t length) const;

  // config detail: {'bit': 16, 'is_signed_var': False, 'len': 16, 'name':
  // 'fMotMaxTq', 'offset': 0.0, 'order': 'intel', 'physical_range':
  // '[0|65535]', 'physical_unit': 'NM', 'precision': 1.0, 'type': 'int'}
