...
void Id0x0c08a7f0::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  chassis->mutable_diamond()->mutable_id_0x0c08a7f0()->set_fmottq(fmottq(bytes, length));
  chassis->mutable_diamond()->mutable_id_0x0c08a7f0()->set_fmotspd(fmotspd(bytes, length));
  chassis->mutable_diamond()->mutable_id_0x0c08a7f0()->set_fmotcur(fmotcur(bytes, length));
  chassis->mutable_diamond()->mutable_id_0x0c08a7f0()->set_bymcusts(bymcusts(bytes, length));
  chassis->mutable_diamond()->mutable_id_0x0c08a7f0()->set_bymcu1msglife1(bymcu1msglife1(bytes, length));
}
...
// config detail: {'bit': 48, 'is_signed_var': False, 'len': 8, 'name': 'bymcusts', 'offset': 0.0, 'order': 'intel', 'physical_range': '[0|255]', 'physical_unit': '', 'precision': 1.0, 'type': 'int'}
...
int Id0x0c08a7f0::bymcu1msglife1(const std::uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 7);
  int32_t x = t0.get_byte(0, 8);
