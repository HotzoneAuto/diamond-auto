...
void Id0x01::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  chassis->mutable_diamond()->mutable_id_0x01()->set_angle_sensor_front(angle_sensor_front(bytes, length));
}
...
double Id0x01::angle_sensor_front(const std::uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 1);
