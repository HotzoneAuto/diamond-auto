...
void ControlComponent::GenerateCommand() {
  ...
  // 初始化前后磁导航检测到的偏差值
  float front_lat_dev_mgs =
      0;  // TODO: lateral_dev_mgs need to changed according to MGS module.
  ...
  if (ID_sensor == 1) {
    front_encoder_angle_realtime = angle_sensor.angle_sensor_data(
        0, 8);  // 前编码器当前的瞬时角度值，单位是deg
  } else if (ID_sensor == 2) {
    rear_encoder_angle_realtime = angle_sensor.angle_sensor_data(
        0, 8);  // 后编码器当前的瞬时角度值，单位是deg
  }
  ...
  */
  ...
  while (true) {
    ...
    */
    ...
    if (/* motor_speed == 1 && */ drivemotor_flag ==
        1)  // 若驱动电机反转（车辆前进A到B）
    {
      ...
      if (front_lat_dev_mgs < -4.5)  //若前方磁导航检测出车偏左
      {
        ...
        */
        cmd->set_front_steering_target(
            10);  // TODO 宗宝一起确定: 10%，待标定（目标角度暂时不用）
      } else if (front_lat_dev_mgs > 4.5)  //若前方磁导航检测出车偏右
      {
        ...
        */
        ...
        if (front_wheel_angle_realtime >= 0.5)  // 当前前轮转角为正，向右偏
        {
          ...
          /*
          Xavier向前变频器发送：向前转向电机发送反转命令：0B 06 10 00 00 02 0C
          61，同时发送额定转速命令：0B 06 20 00 27 10 98 9C
          （也可调速，后期标定） 同时Xavier向四合一id_0x0C079AA7 发送 AA AA AA
          55 AA AA AA AA，逆变器1工作，前转向电机风扇1工作
          */
          cmd->set_front_steering_target(
              -10);  // TODO 宗宝一起确定:
                     // 目标转角为0，摆正，待标定（目标角度暂时不用）
        ...
        } else  // 当前前轮转角为负，向左偏
        {
          ...
          /*
          Xavier向前变频器发送：向前转向电机发送正转命令：0B 06 10 00 00 01 4C
          60，同时发送额定转速命令：0B 06 20 00 27 10 98 9C
          （也可调速，后期标定） 同时Xavier向四合一id_0x0C079AA7 发送 AA AA AA
          55 AA AA AA AA，逆变器1工作，前转向电机风扇1工作
          */
          cmd->set_front_steering_target(
      ...
      } else if (front_lat_dev_mgs > 4.5)  //若后方磁导航检测出车偏右
      {
        ...
        */
        ...
        if (rear_wheel_angle_realtime >= 0.5)  // 当前后轮转角为正，向右偏
        {
          ...
          /*
          Xavier向后变频器发送：向后转向电机发送反转命令：0C 06 10 00 00 02 0D
          D6，同时发送额定转速命令：0C 06 20 00 27 10 99 2B
          （也可调速，后期标定） 同时Xavier向四合一 id_0x0C079AA7 发送 AA AA 55
          AA AA AA AA AA，逆变器2工作，后转向电机风扇2工作
          */
          cmd->set_back_steering_target(
              -10);  // TODO 宗宝一起确定:
                     // 目标转角为0，摆正，待标定（目标角度暂时不用）
        ...
        } else  // 当前后轮转角为负，向左偏
        {
          ...
          /*
          Xavier向后变频器发送：向后转向电机发送正转命令：0C 06 10 00 00 01 4D
          D7，同时发送额定转速命令：0C 06 20 00 27 10 99 2B
          （也可调速，后期标定） 同时Xavier向四合一 id_0x0C079AA7 发送 AA AA 55
          AA AA AA AA AA，逆变器2工作，后转向电机风扇2工作
          */
          cmd->set_back_steering_target(
