...
bool ControlComponent::Init()
{
	...
	magnetic_reader_ = node_->CreateReader<Magnetic>(
	                      FLAGS_magnetic_channel, [this](const std::shared_ptr<Magnetic>& magnetic)
	{
	...
	rfid_reader_ = node_->CreateReader<RFID>(
	                      FLAGS_rfid_topic, [this](const std::shared_ptr<RFID>& rfid)
	{
		rfid_.CopyFrom(*rfid);
	});
...
void ControlComponent::GenerateCommand()
{
	...
	// 初始化驱动电机死区
	if (veh_spd <= 0.1){
		speed_motor_deadzone = speed_motor_deadzone_calibration; // TODO: 落地后标定值
	}
	else{
	...
	*/
	...
	while (true)
	{
		...
		if(rfid_.id() == 2){//检测到B点rfid
			...
			drivemotor_torque = pid_speed(veh_spd, FLAGS_desired_v,speed_motor_deadzone); //驱动电机驱动汽车以1m/s运动，这是PID目标，尽可能接近1
			cmd -> set_throttle(drivemotor_torque);
		}
