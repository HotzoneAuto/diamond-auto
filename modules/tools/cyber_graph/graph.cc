...
//   CyberTopologyMessage topologyMsg(val);
  ...
  auto channelManager =
      apollo::cyber::service_discovery::TopologyManager::Instance()
          ->channel_manager();
  channelManager->AddChangeListener([&attr](const apollo::cyber::proto::ChangeMsg& change_msg) {
        if (change_msg.change_type() == apollo::cyber::proto::ChangeType::CHANGE_PARTICIPANT &&
            change_msg.operate_type() == apollo::cyber::proto::OperateType::OPT_JOIN &&
            change_msg.role_type() == apollo::cyber::proto::RoleType::ROLE_PARTICIPANT) {
