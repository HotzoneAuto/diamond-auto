...
static int32_t setExposureTime(ICameraPtr& cameraSptr, double dExposureTime,
...
// 获取增益值范围
// Get gain range
static int32_t getGainRawMinMaxValue(ICameraPtr& cameraSptr, double& dMinValue,
                                     double& dMaxValue) {
  bool bRet;
  IAnalogControlPtr sptrAnalogControl =
      CSystem::getInstance().createAnalogControl(cameraSptr);
...
static int32_t setBalanceRatio(ICameraPtr& cameraSptr, double dRedBalanceRatio,
...
static int32_t getBalanceRatio(ICameraPtr& cameraSptr, double& dRedBalanceRatio,
                               double& dGreenBalanceRatio,
                               double& dBlueBalanceRatio) {
  bool bRet;
  IAnalogControlPtr sptrAnalogControl =
...
void LogPrinterFunc(const char* log) { return; }
...
// and is used to display the device list** int main()
// {
//  #if 0
...
// cameraIndex = selectDevice(vCameraPtrList.size()); 	cameraSptr =
// vCameraPtrList[cameraIndex];

// 	// GigE相机时，连接前设置相机Ip与网卡处于同一网段上
// 	// When a GigE camera is connected, set the camera IP to be on the same
// network segment as the network card 	if( ICamera::typeGige ==
// cameraSptr->getType())
// 	{
...
//     dBlueBalanceRatio); getBalanceRatioMinMaxValue(cameraSptr,
//     dMinBalanceRatio, dMaxBalanceRatio);
...
//     CThread::getCurrentThreadID()); return 1;
// }
