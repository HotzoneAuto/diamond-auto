...
StreamRetrieve::StreamRetrieve(IStreamSourcePtr& streamSptr)
    : CThread("streamRetrieve"), m_isLoop(false), m_streamSptr(streamSptr) {}
...
void StreamRetrieve::threadProc() {
  while (m_isLoop) {
    ...
    // This frame object must be a temporary variable, and its lifecycle is
    // related to the drive frame cache. The end of this object's life cycle
    // means that the driver can reclaim this frame cache and use it to store
    // subsequent acquired frames. If it is not released in time, the acquired
    // frame will be the same as the set frame cache, and the frame will not be
    // acquired (because the driver has no available frame cache).
    CFrame frame;
