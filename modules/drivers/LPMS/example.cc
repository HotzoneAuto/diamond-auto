...
void pollLoop(std::reference_wrapper<ZenClient> client) {
  while (!g_terminate) {
    ...
    while (true) {
        ...
        switch (event.eventType) {
          case ZenImuEvent_Sample:
            if (i++ % 100 == 0) {
              ...
              std::cout << "> Event component: "
                        << uint32_t(event.component.handle) << std::endl;
      ...
      } else if ((g_gnssHandle > 0) &&
                 (event.component.handle == g_gnssHandle)) {
        switch (event.eventType) {
          case ZenGnssEvent_Sample:
            std::cout << "Event type: " << event.eventType << std::endl;
            std::cout << "> Event component: "
                      << uint32_t(event.component.handle) << std::endl;
            std::cout << "> GPS Fix: \t = " << event.data.gnssData.fixType
                      << std::endl;
            std::cout << "> Longitude: \t = " << event.data.gnssData.longitude
                      << "   Latitude: \t = " << event.data.gnssData.latitude
                      << std::endl;
            ...
            if (event.data.gnssData.carrierPhaseSolution ==
                ZenGnssFixCarrierPhaseSolution_None) {
              std::cout << " > RTK not used" << std::endl;
            } else if (event.data.gnssData.carrierPhaseSolution ==
                       ZenGnssFixCarrierPhaseSolution_FloatAmbiguities) {
              std::cout << " > RTK used with float ambiguities" << std::endl;
            } else if (event.data.gnssData.carrierPhaseSolution ==
                       ZenGnssFixCarrierPhaseSolution_FixedAmbiguities) {
...
int main(int argc, char* argv[]) {
  ...
  for (unsigned idx = 0; idx < g_discoveredSensors.size(); ++idx)
    std::cout << idx << ": " << g_discoveredSensors[idx].name << " ("
              << g_discoveredSensors[idx].ioType << ")" << std::endl;

  unsigned int idx;
  do {
    std::cout << "Provide an index within the range 0-"
              << g_discoveredSensors.size() - 1 << ":" << std::endl;
    std::cin >> idx;
  ...
  auto sensorModelPair =
      sensor.getStringProperty(ZenSensorProperty_SensorModel);
  ...
  if (hasGnss) {
    ...
    /*
    if (gnss.forwardRtkCorrections("RTCM3Network", "192.168.1.117", 9000) !=
    ZenError_None) { std::cout << "Cannot set RTK correction forwarding" <<
    std::endl;
    }
    else {
    ...
    */
    ...
    /*
    if (gnss.forwardRtkCorrections("RTCM3Serial", "COM11", 57600) !=
    ZenError_None) { std::cout << "Cannot set RTK correction forwarding" <<
    std::endl;
    }
    else {
