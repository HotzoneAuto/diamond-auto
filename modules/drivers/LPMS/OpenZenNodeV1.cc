...
class OpenZenSensor {
 public:
  ...
  OpenZenSensor(apollo::cyber::Node node)
    ...
    // using 0 as default will tell OpenZen to use the defaul baudrate for a
    // respective sensor
    ...
    // convention
    private_nh.param<bool>("use_lpms_acceleration_convention",
    ...
    auto autocalibration_serv = node_->CreateService<Content, Content>(
        "auto_calibration", &OpenZenSensor::autocalibration, this);
    auto gyrocalibration_serv = node_->CreateService<Content, Content>(
        "gyro_calibration", &OpenZenSensor::gyrocalibration, this);
    auto resetHeading_serv = node_->CreateService<Content, Content>(
        "reset_heading", &OpenZenSensor::resetHeading, this);
  ...
  bool setAutocalibration(std_srvs::SetBool::Request& req,
    ...
    if (auto error = m_zenImu->setBoolProperty(
            ...
            msg.append(std::string(
                           "[Failed] current autocalibration status set to: ") +
                       (req.data ? "True" : "False"));
  ...
  bool calibrateGyroscope(std_srvs::Trigger::Request& req,
...
int main(int argc, char* argv[]) {
  ...
  if (!lpOpenZen.run()) {
    auto reset_heading_client =
        node->CreateClient<Content, Content>("reset_heading");
    auto auto_calibration_client =
        node->CreateClient<Content, Content>("auto_calibration");
    auto gyro_calibration_client =
        node->CreateClient<Content, Content>("gyro_calibration_client");
    ...
    if (reset_heading_res != nullptr) {
      AINFO << "reset_heading_client: responese: "
            << reset_heading_res->ShortDebugString();
    } else {
      AINFO << "reset_heading_client: service may not ready.";
    }

    if (auto_calibration_res != nullptr) {
      AINFO << "auto_calibration_client: response: "
            << auto_calibration_res->ShortDebugString();
    } else {
      AINFO << "auto_calibration_client: service may not ready.";
    }

    if (gyro_calibration_res != nullptr) {
      AINFO << "gyro_calibration_client: response: "
            << gyro_calibration_res->ShortDebugString();
    } else {
