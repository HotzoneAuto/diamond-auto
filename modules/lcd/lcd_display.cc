...
void MessageCallback(
  ...
  device_ = std::make_unique<Uart>("ttyUSB1");
  device_ -> SetOpt(9600, 8, 'N', 1);
  unsigned char vehicle_id[8] = {0x01, 0x06, 0x00, 0x00,
                                    0x00, 0x01, 0x48, 0x0A};
  device_->Write(vehicle_id, 8);
  AINFO << "vehicle_id->" << msg->vehicle_id().vin();
  // AINFO << "results->" << results;
  AINFO << "speed_mps-> " << msg ->speed_mps();
  int speed=round(3.6*msg->speed_mps());

  unsigned char c0[8] = {0x01, 0x06, 0x00, 0x01,
                        0x00, 0x00, 0xD8, 0x0A};

  unsigned char c1[8] = {0x01, 0x06, 0x00, 0x01,
                        0x00, 0x01, 0x19, 0xCA};

  unsigned char c2[8] = {0x01, 0x06, 0x00, 0x01,
                         0x00, 0x02, 0x59, 0xCB};

  unsigned char c3[8] = {0x01, 0x06, 0x00, 0x01,
                        0x00, 0x03, 0x98, 0x0B};

  unsigned char c4[8] = {0x01, 0x06, 0x00, 0x01,
                        0x00, 0x04, 0xD9, 0xC9}; 
  
  unsigned char c5[8] = {0x01, 0x06, 0x00, 0x01,
                        0x00, 0x05, 0x18, 0x09};

  unsigned char c6[8] = {0x01, 0x06, 0x00, 0x01,
                        0x00, 0x06, 0x58, 0x08};

  unsigned char c7[8] = {0x01, 0x06, 0x00, 0x01,
                        0x00, 0x07, 0x99, 0xC8};

  unsigned char c8[8] = {0x01, 0x06, 0x00, 0x01,
                         0x00, 0x08, 0xD9, 0xCC};
  ...
  }else if(msg->driving_mode()==1 or msg->driving_mode()==2 or msg->driving_mode()==3){
    unsigned char driving_mode[8] = {0x01, 0x06, 0x00, 0x02,
                                     0x00, 0x00, 0x28, 0x0A};
    device_->Write(driving_mode, 8);
  }
